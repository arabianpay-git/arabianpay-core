<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ translate('Create API Token') }}
        </x-slot>

        <x-slot name="description">
            {{ translate('API tokens allow third-party services to authenticate with our application on your behalf.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ translate('Token Name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ translate('Permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                ...
                <x-slot name="description">
                    {{ translate('You may delete any of your existing tokens if they are no longer needed.') }}
                </x-slot>
                                ...
                                <div class="flex items-center ms-2">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ translate('Last used') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ translate('Please copy your new API token. For your security, it won\'t be shown again.') }}
            </div>
    ...
    <x-dialog-modal wire:model.live="managingApiTokenPermissions">
        <x-slot name="title">
            {{ translate('API Token Permissions') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ translate('Delete API Token') }}
        </x-slot>

        <x-slot name="content">
            {{ translate('Are you sure you would like to delete this API token?') }}
        </x-slot>
