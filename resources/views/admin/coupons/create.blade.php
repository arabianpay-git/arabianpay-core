                        ...
                        <div class="card-body grid gap-5">
                            ...
                            <div class="w-full">
                                <div class="flex items-baseline flex-wrap gap-2.5">
                                    ...
                                    <input class="input @error('code') border-red-500 @enderror" name="code" type="text" value="{{ old('code') }}" required />
                                </div>
                                @error('code')
                                <span class="text-danger text-sm">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="w-full">
                                <div class="flex items-baseline flex-wrap gap-2.5">
                                    <label class="form-label flex items-center gap-1 max-w-56">Coupon Type</label>
                                    <select class="input @error('type') border-red-500 @enderror" name="type" id="coupon_type" required>
                                        <option value="total_order" {{ old('type') == 'total_order' ? 'selected' : '' }}>Total Order</option>
                                        <option value="product_base" {{ old('type') == 'product_base' ? 'selected' : '' }}>Product Base</option>
                                    </select>
                                </div>
                                @error('type')
                                <span class="text-danger text-sm">{{ $message }}</span>
                            ...
                            <div class="w-full" id="total_order_section" style="display: none;">
                                <div class="flex flex-col gap-4">
                                    <div class="flex items-baseline flex-wrap gap-2.5">
                                        ...
                                        <input type="number" class="input" name="min_order_amount" step="0.01" value="{{ old('min_order_amount') }}">
                                    </div>
                    
                                    <div class="flex items-baseline flex-wrap gap-2.5">
                                        <label class="form-label flex items-center gap-1 max-w-56">Maximum Discount</label>
                                        <input type="number" class="input" name="max_discount_amount" step="0.01" value="{{ old('max_discount_amount') }}">
                                    </div>
                                </div>
                            </div>
                    
                            <div class="grid grid-cols-2 gap-5">
                                <input class="input" name="discount" type="number" placeholder="Discount" value="{{ old('discount') }}" required />
                            ...
                            <div class="grid grid-cols-2 gap-5">
                                <input class="input" name="start_date" type="date" placeholder="Discount Start Date" value="{{ old('start_date') }}">
                                <input class="input" name="end_date" type="date" placeholder="Discount End Date" value="{{ old('end_date') }}">
                            </div>
