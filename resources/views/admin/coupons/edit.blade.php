                    ...
                    <div class="card pb-2.5">
                        ...
                        <form action="{{ route('coupons.update', $coupon->id) }}" method="POST">
                            ...
                            <div class="card-body grid gap-5">
                                        ...
                                        <select class="input @error('user_id') border-red-500 @enderror" name="user_id"
                                            ...
                                            @foreach ($merchants as $merchant)
                                                <option value="{{ $merchant->id }}"
                                                    {{ old('user_id', $coupon->user_id) == $merchant->id ? 'selected' : '' }}>
                                ...
                                <div class="w-full">
                                    <div class="flex items-baseline flex-wrap gap-2.5">
                                        ...
                                        <input class="input @error('code') border-red-500 @enderror" name="code"
                                            type="text" value="{{ old('code', $coupon->code) }}" required />
                                    </div>
                                    @error('code')
                                        <span class="text-danger text-sm">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="w-full">
                                    ...
                                    <select class="input @error('type') border-red-500 @enderror" name="type"
                                        ...
                                        <option value="total_order"
                                            {{ old('type', $coupon->type) == 'total_order' ? 'selected' : '' }}>
                                            {{ translate('Total Order') }}</option>
                                        <option value="product_base"
                                            {{ old('type', $coupon->type) == 'product_base' ? 'selected' : '' }}>
                                ...
                                <div class="w-full" id="product_select_section"
                                    style="display: {{ $coupon->type === 'product_base' ? 'block' : 'none' }}">
                                ...
                                <div class="w-full" id="total_order_section"
                                    style="display: {{ $coupon->type === 'total_order' ? 'block' : 'none' }}">
                                    <div class="flex flex-col gap-4">
                                        <div class="flex items-baseline flex-wrap gap-2.5">
                                            ...
                                            <input type="number" class="input" name="min_order_amount" step="0.01"
                                                value="{{ old('min_order_amount', $coupon->details['min_order_amount'] ?? '') }}">
                                        </div>
                                        ...
                                        <div class="flex items-baseline flex-wrap gap-2.5">
                                            ...
                                            <input type="number" class="input" name="max_discount_amount" step="0.01"
                                                value="{{ old('max_discount_amount', $coupon->details['max_discount_amount'] ?? '') }}">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-5">
                                    <input class="input" name="start_date" type="text"
                                        placeholder="{{ translate('Discount Start Date') }}"
                                        value="{{ old('start_date', $coupon->start_date) }}">
                                    <input class="input" name="end_date" type="text"
