                            ...
                            <table class="table table-auto table-border" data-datatable-table="true">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">
                                            <span class="sort">
                                                <span class="sort-label font-normal text-gray-700">Supplier Amount</span>
                                                <span class="sort-icon"> </span>
                                            </span>
                                        </th>
                                        ...
                                        <th class="text-center">
                                            <span class="sort">
                                                <span class="sort-label font-normal text-gray-700">Total Payment</span>
                                                <span class="sort-icon"> </span>
                                            </span>
                                        </th>
                                        ...
                                        <th class="text-center">
                                            <span class="sort">
                                                <span class="sort-label font-normal text-gray-700">Total Amount</span>
                                                <span class="sort-icon"> </span>
                                            </span>
                                        </th>
                                    ...
                                    @foreach ($transactions as $transaction)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <br>
                                                <small class="text-gray-500">— {{ $transaction->user?->business_name ?? '' }}</small>
                                            </td>
                                
                                            <td>
                                                {{ $transaction->seller?->first_name ?? 'N/A' }} {{ $transaction->seller?->last_name ?? 'N/A' }}
                                                <br>
                                                <small class="text-gray-500">— {{ $transaction->seller?->business_name ?? '' }}</small>
                                            </td>
                                
                                            <td>{{ $transaction->order?->tracking ?? 'N/A' }}</td>
                                            ...
                                            <td class="text-center">
                                                <span class="badge badge-sm badge-{{ $transaction->settlement_status == 'settled' ? 'success' : 'danger' }} badge-outline">
                                            ...
                                            <td>
                                                <div class="flex gap-1">
                                                    <a class="btn btn-sm btn-icon btn-clear btn-primary" href="{{ route('orders.details', ['id' => $transaction->order->id]) }}">
...
<div class="modal" data-modal="true" id="payment_modal">
    <div class="modal-content max-w-[600px] top-[5%]">
        <div class="modal-header py-4 px-5">
            <h5 class="modal-title">Make Supplier Payment</h5>
        ...
        <form action="{{ route('sellerPaymentFromAdmin') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body grid gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" required class="input @error('amount') border-red-500 @enderror" placeholder="Enter amount" />
                    @error('amount')
                ...
                <div>
                    ...
                    <input type="text" name="transfer_number" required class="input @error('transfer_number') border-red-500 @enderror" placeholder="Enter transfer number" />
                ...
                <div>
                    ...
                    <input type="date" name="payment_date" id="modal_payment_date" required class="input @error('payment_date') border-red-500 @enderror" />
                    @error('payment_date')
                    <span class="text-danger text-sm">{{ $message }}</span>
                    @enderror
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.open-payment-modal').forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.getElementById('modal_transaction_id').value = this.dataset.id || 'N/A';
            document.getElementById('modal_seller_id').value = this.dataset.sellerId || 'N/A';
            document.getElementById('modal_due').textContent = formatValue(this.dataset.due);
            document.getElementById('modal_transfer_fee').textContent = formatValue(this.dataset.transferFee);
            document.getElementById('modal_final').textContent = formatValue(this.dataset.final);
            document.getElementById('modal_settled').textContent = formatValue(this.dataset.settled);
            document.getElementById('modal_refund').textContent = formatValue(this.dataset.refundSum);
            
            // Calculate remaining amount
            const remaining = (parseFloat(this.dataset.due) || 0) - 
                            (parseFloat(this.dataset.settled) || 0) - 
                            (parseFloat(this.dataset.refundSum) || 0);
            document.getElementById('modal_remaining').textContent = formatValue(remaining.toFixed(2));
            
            document.getElementById('modal_payment_date').value = this.dataset.date || new Date().toISOString().split('T')[0];
        });
    });
