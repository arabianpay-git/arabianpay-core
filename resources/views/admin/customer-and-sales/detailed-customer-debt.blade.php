                    ...
                    <form method="GET" action="{{ route('detailedCustomerDebt') }}">
                        <div class="flex flex-wrap items-center gap-4">
                            <div class="w-full md:w-1/3">
                                ...
                                <select id="customer_select" name="customer_id" class="form-select w-full">
                                    ...
                                    @foreach ($customers as $customer)
                                        <option value="{{ encrypt($customer['id']) }}"
                                            {{ request('customer_id') == encrypt($customer['id']) ? 'selected' : '' }}>
                            ...
                            <div class="pt-5 md:pt-0 flex items-center gap-3">
                                ...
                                <a href="{{ route('detailedCustomerDebt') }}" class="btn btn-danger btn-sm">
                        ...
                        <div class="flex flex-wrap gap-2 lg:gap-5">
                            <div class="flex">
                                <label class="input input-sm">
                                    ...
                                    <input data-datatable-search="#customer_collection_table"
                                        placeholder="{{ translate('Search customers') }}" type="text" />
                                </label>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table table-auto table-border" data-datatable-table="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-left">{{ translate('Tax Number') }}</th>
                                            <th class="text-center">{{ translate('Total Invoices') }}</th>
                                            <th class="text-center">{{ translate('Total Collected') }}</th>
                                            <th class="text-center">{{ translate('Remaining Balance') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                                ...
                                                <td>
                                                    <div class="whitespace-nowrap">
                                                        ...
                                                        <small class="text-gray-500">—
                                                            {{ $item['customer_business'] }}</small>
                                                    </div>
                                                </td>
                                                <td>{{ $item['tax_number'] }}</td>
                                                <td class="text-center"><span class="icon-saudi_riyal"></span>
                                                    {{ number_format((float) $item['total_invoice'], 2) }}</td>
                                                <td class="text-center"><span class="icon-saudi_riyal"></span>
                                                    {{ number_format((float) $item['total_collected'], 2) }}</td>
                                                <td class="text-center"><span class="icon-saudi_riyal"></span>
