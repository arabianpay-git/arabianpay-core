        ...
        <div class="card max-w-[370px] w-full">
            <form action="{{ route('otp.verify.confirm') }}" method="POST" class="card-body flex flex-col gap-5 p-10"
                ...
                <div class="text-center mb-2.5">
                    <h3 class="text-lg font-medium text-gray-900">{{ translate('Enter OTP Code') }}</h3>
                    <p class="text-sm text-gray-500">
                        {{ translate('A 6-digit code was sent to') }} <strong>{{ $phone }}</strong>
                    </p>
                </div>
                ...
                <div class="text-center mt-2 text-sm text-gray-500">
                    <span id="otp_expire_timer" class="font-medium">{{ translate('OTP expires in:') }} --:--</span>
                </div>

                {{-- Submit Button --}}
                <button type="submit" class="btn btn-primary w-full mt-4 flex justify-center items-center" id="submit_btn">
                    <span id="submit_text">{{ translate('Verify OTP') }}</span>
