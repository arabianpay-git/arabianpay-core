            ...
            <tbody id="schedulePaymentsBody">
                @forelse($checkout->schedulePayments ?? [] as $index => $payment)
                    @php
                        ...
                        $isDueThisWeek =
                            $payment->due_date->between(now()->startOfWeek(), now()->endOfWeek()) &&
                            $payment->payment_status !== 'paid';
                    @endphp
                    <tr class="schedule-payment-row" data-status="{{ $payment->payment_status }}"
                        data-due-date="{{ $payment->due_date->format(dateFormat()) }}"
                        data-amount="{{ $payment->instalment_amount }}" data-days="{{ $daysFromDue }}">
                        ...
                        <td>
                            <div class="flex items-center gap-3">
                                <div
                                    class="flex items-center justify-center w-10 h-10 {{ $payment->payment_status === 'paid' ? 'bg-success-light' : ($isOverdue ? 'bg-danger-light' : 'bg-warning-light') }} rounded-full">
                                    <i
                                        class="ki-filled {{ $payment->payment_status === 'paid' ? 'ki-check text-success' : ($isOverdue ? 'ki-cross-circle text-danger' : 'ki-time text-warning') }} text-base"></i>
                                </div>
                        ...
                        <td>
                            <div class="flex flex-col">
                                <span
                                    class="text-sm font-medium {{ $isOverdue ? 'text-danger' : ($isDueToday ? 'text-warning' : 'text-gray-900') }}">
                        ...
                        <td>
                            <div class="flex flex-col">
                                ...
                                @elseif($isOverdue)
                                    <span class="text-sm text-danger font-medium">{{ abs(round($daysFromDue)) }} days
                                        overdue</span>
                                @elseif($isDueToday)
                                    <span class="text-sm text-warning font-medium">Due today</span>
                                @elseif($daysFromDue > 0)
                                    <span class="text-sm text-info font-medium">{{ round($daysFromDue) }} days
                                        left</span>
                                @else
                        ...
                        <td>
                            ...
                            @if ($claimsCount > 0)
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-xs text-gray-600">{{ $claimsCount }}
                                            claim{{ $claimsCount > 1 ? 's' : '' }}</span>
                                        @if ($lastClaim)
                                            <span class="text-xs text-gray-500">Last:
                                                {{ $lastClaim->created_at->diffForHumans() }}</span>
                                        @endif
                                    </div>
                                </div>
                            @else
                        ...
                        <td>
                            @if ($payment->payment && $payment->payment->created_at)
                                <div class="flex flex-col">
                                    <span
                                        class="text-sm text-success">{{ $payment->payment->created_at->format('M d, Y') }}</span>
                                    <span
                                        class="text-xs text-gray-600">{{ $payment->payment->created_at->format('h:i A') }}</span>
                                </div>
                            @else
                        ...
                        <td class="text-center">
                            <div class="dropdown" data-dropdown="true">
                                <button class="dropdown-toggle btn btn-sm btn-icon btn-light btn-clear"
                                ...
                                <div class="dropdown-content menu-default w-full max-w-[200px]">
                                    ...
                                    @if ($payment->payment_status !== 'paid')
                                        <div class="menu-item">
                                            <a class="menu-link" href="#"
                                                ...
                                                <span class="menu-icon"><i
                                                        class="ki-filled ki-check text-success"></i></span>
                                        ...
                                        <div class="menu-item">
                                            <a class="menu-link" href="#"
                                                onclick="createClaim({{ $payment->id }}, 'call')">
                                                <span class="menu-icon"><i
                                                        class="ki-filled ki-phone text-primary"></i></span>
                                        ...
                                        <div class="menu-item">
                                            <a class="menu-link" href="#"
                                                onclick="createClaim({{ $payment->id }}, 'sms')">
                                                <span class="menu-icon"><i
                                                        class="ki-filled ki-sms text-info"></i></span>
                                        ...
                                        @if ($isOverdue)
                                            <div class="menu-item">
                                                <a class="menu-link" href="#"
                                                    ...
                                                    <span class="menu-icon"><i
                                                            class="ki-filled ki-notification text-warning"></i></span>
                                    ...
                                    @if ($claimsCount > 0)
                                        ...
                                        <div class="menu-item">
                                            <a class="menu-link" href="#"
                                                onclick="viewClaims({{ $payment->id }})">
                                                <span class="menu-icon"><i
                                                        class="ki-filled ki-document text-secondary"></i></span>
                    ...
                    <tr id="no-payments-row">
                        <td colspan="9" class="text-center py-10">
                            <div class="flex flex-col items-center">
                                ...
                                <div class="text-gray-500 text-sm">This checkout doesn't have any scheduled payments
                                    yet</div>
                            </div>
                        </td>
                    </tr>
    ...
    <div class="flex items-center gap-6 text-sm">
        <div>
            ...
            <span class="font-semibold text-gray-900"
                id="totalCount">{{ $checkout->schedulePayments->count() }}</span>
        </div>
        <div>
            <span class="text-gray-600">Total Amount:</span>
            <span class="font-semibold text-gray-900"
                id="totalAmount">{{ number_format($checkout->schedulePayments->sum('instalment_amount'), 2) }}
                SAR</span>
        </div>
        <div>
            <span class="text-gray-600">Paid:</span>
            <span class="font-semibold text-success"
                id="collectedAmount">{{ number_format($checkout->schedulePayments->where('payment_status', 'paid')->sum('instalment_amount'), 2) }}
                SAR</span>
        </div>
        <div>
            <span class="text-gray-600">Outstanding:</span>
            <span class="font-semibold text-warning"
                id="outstandingAmount">{{ number_format($checkout->schedulePayments->where('payment_status', '!=', 'paid')->sum('instalment_amount'), 2) }}
                SAR</span>
        </div>
    </div>
    <div class="text-sm text-gray-500">
        Showing <span id="visibleCount">{{ $checkout->schedulePayments->count() }}</span> of
        {{ $checkout->schedulePayments->count() }} payments
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterAndSortScheduleTable() {
            ...
            let visibleRows = rows.filter(row => {
                ...
                // Search filter
                if (searchTerm && !amount.includes(searchTerm) && !row.textContent.toLowerCase()
                    .includes(searchTerm)) {
                    return false;
                }
    ...
    `;
        ...
        fetch(jsonUrl)
            ...
            .then(({
                ...
                const late = schedule.is_late ?
                    `<span style=\"color:#dc2626;font-size:12px;margin-left:8px;\">${schedule.late_days ?? 0} days late</span>` :
                    '';
