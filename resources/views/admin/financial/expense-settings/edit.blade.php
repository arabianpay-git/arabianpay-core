            ...
            <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
                <div class="flex flex-col justify-center gap-2">
                    <h1 class="text-xl font-medium leading-none text-gray-900">
                        {{ translate('Edit Expense Setting') }}
                    </h1>
                    <div class="flex items-center gap-2 text-sm font-medium text-gray-600">
                        ...
                        <span class="text-gray-900">{{ translate('Edit') }}</span>
                    </div>
                </div>
        ...
        <div class="container-fixed">
            <div class="grid gap-5 lg:gap-7.5">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('financial.expense-settings.update', $expenseSetting->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-7.5">
                                ...
                                <div class="flex flex-col gap-2.5">
                                    ...
                                    <label class="input">
                                        <input name="refrence_id" type="text" placeholder="{{ translate('Enter reference ID') }}" 
                                               value="{{ old('refrence_id', $expenseSetting->refrence_id) }}" required />
                                    </label>
                                    @error('refrence_id')
                                ...
                                <div class="flex flex-col gap-2.5">
                                    ...
                                    <select name="amount_type" id="amount_type" class="select" required>
                                        ...
                                        <option value="fixed" {{ old('amount_type', $expenseSetting->amount_type) == 'fixed amount' ? 'selected' : '' }}>{{ translate('Fixed Amount') }}</option>
                                        <option value="percent" {{ old('amount_type', $expenseSetting->amount_type) == 'percent' ? 'selected' : '' }}>{{ translate('Percentage') }}</option>
                                    </select>
                                    @error('amount_type')
                                        <span class="form-hint text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="flex flex-col gap-2.5">
                                    ...
                                    <label class="input">
                                        <input name="amount" id="amount" type="number" step="0.01" min="0" 
                                               ...
                                               value="{{ old('amount', $expenseSetting->amount) }}" required />
                                ...
                                <div class="flex flex-col gap-2.5">
                                    ...
                                    <select name="credit_acc_id" class="select" required>
                                        ...
                                        @foreach($creditAccounts as $account)
                                            <option value="{{ $account->id }}" {{ old('credit_acc_id', $expenseSetting->credit_acc_id) == $account->id ? 'selected' : '' }}>
                                ...
                                <div class="flex flex-col gap-2.5 lg:col-span-2">
                                    ...
                                    <textarea name="description" class="textarea" rows="4" 
                                              placeholder="{{ translate('Enter description') }}" required>{{ old('description', $expenseSetting->description) }}</textarea>
                                    @error('description')
                                        <span class="form-hint text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="flex justify-end gap-4 mt-7.5 mt-5">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    {{ translate('Update Expense Setting') }}
                                </button>
                            </div>
                        </form>
                    </div>
