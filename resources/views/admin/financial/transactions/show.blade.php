                    ...
                    <div class="card-body">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-7.5">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <span
                                    class="text-gray-900 font-medium text-lg">SR{{ number_format($fTransaction->amount, 2) }}</span>
                            </div>

                            <div class="flex flex-col gap-2">
                                <label class="text-gray-600 text-sm">{{ translate('Status') }}</label>
                                <span
                                    class="badge badge-xs text-xs px-2 py-1 w-fit inline-block 
                                    {{ $fTransaction->status == 'completed'
                                        ? 'badge-success'
                                        : ($fTransaction->status == 'pending'
                                            ? 'badge-warning'
                                            : ($fTransaction->status == 'failed'
                                                ? 'badge-danger'
                                                : 'badge-secondary')) }}">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <span
                                    class="text-gray-900">{{ $fTransaction->transaction_date ? \Carbon\Carbon::parse($fTransaction->transaction_date)->format('Y-m-d H:i:s') : '-' }}</span>
                            </div>
                    ...
                    <div class="card-body">
                                ...
                                <table class="table table-auto table-border">
                                        ...
                                        @foreach ($entries as $entry)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($entry->account)
                                                        <span
                                                            class="text-sm font-medium">{{ $entry->account->account_code }}</span>
                                                        <br>
                                                        <span
                                                            class="text-xs text-gray-600">{{ $entry->account->account_name }}</span>
                                                    @else
                                                        <span
                                                            class="text-gray-400">{{ $entry->account_name ?? '-' }}</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($entry->debit)
                                                        <span
                                                            class="text-red-600 font-medium">SR{{ number_format($entry->debit, 2) }}</span>
                                                    @else
                                                        <span class="text-gray-400">-</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($entry->credit)
                                                        <span
                                                            class="text-green-600 font-medium">SR{{ number_format($entry->credit, 2) }}</span>
                                                    @else
                                                        <span class="text-gray-400">-</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    <span
                                                        class="badge badge-xs text-xs px-2 py-1 w-fit inline-block {{ $entry->status == 'completed' ? 'badge-success' : ($entry->status == 'pending' ? 'badge-warning' : 'badge-danger') }}">
                                    ...
                                    <tfoot>
                                        <tr class="bg-gray-50">
                                            ...
                                            <td class="font-medium text-green-600">SR{{ number_format($totalCredit, 2) }}
                                            </td>
                                            <td colspan="2" class="font-medium">
                                                {{ translate('Balance') }}:
                                                <span
                                                    class="{{ $totalDebit - $totalCredit == 0 ? 'text-green-600' : 'text-red-600' }}">
                        ...
                        @else
                            <div class="flex flex-col items-center gap-3 py-10">
                                ...
                                <span
                                    class="text-gray-600">{{ translate('No journal entries found for this transaction') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
