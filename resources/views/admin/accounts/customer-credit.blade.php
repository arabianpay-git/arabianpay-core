                    ...
                    <div class="card p-4 bg-white dark:bg-gray-800">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-primary-600">{{ $creditScore['compositeScore'] }}/100</p>
                            </div>
...
<script>
    // Score Gauge
    const gaugeOptions = {
        series: [{{ $creditScore['compositeScore'] }}],
        ...
        fill: { type: 'gradient', gradient: {
            shade: 'dark', shadeIntensity: 0.15,
    ...
    const breakdownOptions = {
        series: [{ data: {!! json_encode(array_values($scoreComponents)) !!} }],
    ...
    const timelineOptions = {
        series: [{ name: "On-time Payments", data: {!! json_encode($paymentTimeline['data']) !!} }],
        chart: { height: 350, type: 'line' },
        xaxis: { categories: {!! json_encode($paymentTimeline['categories']) !!} },
        colors: ['#10B981']
    };
