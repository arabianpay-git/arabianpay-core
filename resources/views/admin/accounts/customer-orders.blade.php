                            ...
                            <table class="table table-auto table-border" data-datatable-table="true">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">
                                            <span class="sort">
                                                <span class="sort-label font-normal text-gray-700">Coupon Discount</span>
                                                <span class="sort-icon"> </span>
                                            </span>
                                        </th>
                                        <th class="text-center">
                                            <span class="sort">
                                                <span class="sort-label font-normal text-gray-700">Delivery Status</span>
                                                <span class="sort-icon"> </span>
                                            </span>
                                        </th>
                                        <th class="text-center">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if($item->payment_status == 'completed')
                                                <span class="badge badge-sm badge-outline badge-success">{{ ucfirst($item->payment_status) }}</span>
                                            @elseif($item->payment_status == 'pending')
                                                <span class="badge badge-sm badge-outline badge-warning">{{ ucfirst($item->payment_status) }}</span>
                                            @elseif($item->payment_status == 'failed')
                                                <span class="badge badge-sm badge-outline badge-danger">{{ ucfirst($item->payment_status) }}</span>
                                            @else
                                                <span class="badge badge-sm badge-outline badge-info">{{ ucfirst($item->payment_status) }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center">
                                            @if($item->delivery_status == 'delivered')
                                                <span class="badge badge-sm badge-outline badge-success">{{ ucfirst($item->delivery_status) }}</span>
                                            @elseif($item->delivery_status == 'shipped')
                                                <span class="badge badge-sm badge-outline badge-warning">{{ ucfirst($item->delivery_status) }}</span>
                                            @elseif($item->delivery_status == 'pending')
                                                <span class="badge badge-sm badge-outline badge-primary">{{ ucfirst($item->delivery_status) }}</span>
                                            @else
                                                <span class="badge badge-sm badge-outline badge-info">{{ ucfirst($item->delivery_status) }}</span>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            @if($item->general_status == 'completed')
                                                <span class="badge badge-sm badge-outline badge-success">{{ ucfirst($item->general_status) }}</span>
                                            @elseif($item->general_status == 'Processing')
                                                <span class="badge badge-sm badge-outline badge-warning">{{ ucfirst($item->general_status) }}</span>
                                            @elseif($item->general_status == 'cancelled')
                                                <span class="badge badge-sm badge-outline badge-danger">{{ ucfirst($item->general_status) }}</span>
                                            @else
                                                <span class="badge badge-sm badge-outline badge-info">{{ ucfirst($item->general_status) }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <div class="flex gap-1 justify-center">
                                                <a class="btn btn-sm btn-icon btn-clear btn-primary" href="{{ route('orders.details', ['id' => $item->id]) }}">
