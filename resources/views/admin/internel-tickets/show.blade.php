                                ...
                                <div class="ps-2.5 mb-7 text-md grow">
                                    <div class="flex flex-col">
                                            ...
                                            <div class="text-sm text-gray-800 font-semibold">
                                                ...
                                                <small class="text-xs font-medium text-gray-500">
                                                    @if (\Carbon\Carbon::parse($activity['created_at'])->isToday())
                                                        (Today, {{ \Carbon\Carbon::parse($activity['created_at'])->format('g:i A') }})
                                                    @else
                                        ...
                                        @if (!empty($activity['files']))
                                            <div class="flex flex-wrap gap-2 mt-3">
                                                @foreach ($activity['files'] as $file)
                                                    <a href="{{ $file }}" target="_blank" class="text-xs text-primary underline">
                                                        View Attachment
                                                    </a>
                                                @endforeach
                                            </div>
                                        ...
                                        <div class="mt-2">
                                            <button
                                                type="button"
                                                class="btn btn-link reply-toggle-btn"
                                                data-index="{{ $index }}"
                                            >
                                        ...
                                        <form
                                            action="{{ route('ticketReply', $activity['id']) }}"
                                            method="POST"
