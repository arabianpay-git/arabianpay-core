        ...
        <div class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-1">
            @foreach ($schedulePayments as $index => $payment)
                @php
                    ...
                    // Mark as 'late' if due_date has passed and not paid
                    if ($status !== 'paid' && $dueDateCarbon->lt($today)) {
                        $status = 'late';
                    }
                    ...
                    $remaining = number_format(
                        ($payment->instalment_amount ?? 0) - ($payment->deducted_amount ?? 0),
                        2,
                    );

                    // Check if a promise exists for this installment
                ...
                <div class="sp-card {{ $statusClass }}">
                    <div class="sp-ribbon flex items-center gap-2">
                                ...
                                <div class="sp-title">
                                    ...
                                    @if ($hasPromise)
                                        <div class="inst-sub text-xs text-gray-500">
                                            {{ translate('Original Due:') }} {{ $dueDate }} |
                                            {{ translate('Promise Due:') }} {{ $promiseDueDate }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="sp-body">
                            <div class="sp-row">
                                <div class="meta">
                                    ...
                                    @endif
                                    <span class="meta-badge gray">{{ translate('Remaining') }}:
                                        {{ $remaining }}</span>

                                    @if ($hasPromise)
                                        <span class="meta-badge yellow">
                                            {{ translate('Promise') }}: {{ $promiseDueDate }}
                                        </span>
                                    @endif
                                </div>
            ...
            @include('admin.orders.partials.order-partials')
        </div>
    </div>
</div>
@include('admin.orders.partials.installment-update-modal')
@include('admin.orders.partials.paynow-modal')
@push('styles')
    ...
    <style>
        ...
        /* --- Promise Status (Warning/Yellow) --- */
        .sp-card.promise {
            border-left: 4px solid #f59e0b;
            /* Amber/Warning */
        }

        /* Ribbon color for promise */
        .sp-card.promise .sp-ribbon-text {
            background-color: #f59e0b;
            /* Amber/Warning */
        }

        /* Installment circle color for promise */
        .sp-card.promise .installment-number-circle {
            background-color: #f59e0b;
            /* Amber/Warning */
        }
