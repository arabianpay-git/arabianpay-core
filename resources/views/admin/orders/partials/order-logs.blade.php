            ...
            <div class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-1">
                @foreach ($actionLogs as $log)
                    @php
                        ...
                        // 3. Use the detailed description generated by the controller
                        $formattedDescription = $log->description ?? translate('No description provided.');

                        // 4. Device and metadata info from new properties
                    ...
                    <div class="log-card {{ $eventColor }}">
                        <div class="log-top">
                            {{-- Use action_type for better logging instead of the old 'event' property --}}
                        ...
                        <div class="log-body mt-1 text-sm text-gray-700">
                            {{-- Display the actor label and type --}}
                            <div class="log-user font-semibold">
                                {{ $actorLabel }}
                                <span class="text-xs text-gray-500 font-normal">({{ ucfirst($actorType) }})</span>
                            </div>

                            {{-- Display the detailed description from the controller --}}
