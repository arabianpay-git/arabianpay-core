        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-7.5">
                                        ...
                                        @foreach ($productDetails as $item)
                                            ...
                                            <tr>
                                                <td class="px-4 py-2 border-b">
                                                    <img src="{{ $item['product']->thumbnail ?? 'https://via.placeholder.com/60' }}" alt="Product Image" class="w-16 h-16 object-cover">
                                                </td>
                                                <td class="px-4 py-2 border-b">
                                                    <div class="font-semibold">{{ $item['product']->name ?? 'N/A' }}</div>
                                                    <div class="text-xs text-gray-600">SKU: {{ $item['product']->sku ?? 'N/A' }}</div>
                                                    @foreach ($item['attributes'] as $attribute)
                                                        <div class="text-xs text-gray-600">{{ $attribute['attribute'] }}: {{ $attribute['value'] }}</div>
                                                    @endforeach
                                                    <div class="text-xs text-gray-600">
                                                        ↳ Shipping 
                                                        @if($order->shipping_cost)
                                                            <span class="icon-saudi_riyal"></span> {{ $order->shipping_cost }}
                                                        @else
                                                            Free delivery
                                                        @endif
                                                    </div>
                                                    
                                                    <div class="text-xs text-gray-600">↳ Store {{ $order->user->business_name ?? 'Default Store' }}</div>
                                                </td>
                                                <td class="px-4 py-2 text-right border-b"><span class="icon-saudi_riyal"></span> {{ number_format($item['price'], 2) }}</td>
                                                <td class="px-4 py-2 text-center border-b">{{ $item['quantity'] }}</td>
                                                <td class="px-4 py-2 text-right border-b"><span class="icon-saudi_riyal"></span> {{ number_format($item['total'], 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                                ...
                                <table class="table-auto w-full">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="text-sm text-gray-800">{{ $order->user->first_name }} {{ $order->user->last_name }}</td>
                                        </tr>
                                        <tr>
                                ...
                                <table class="table-auto w-full">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="text-sm text-gray-800">{{ $order->shipping_first_name }} {{ $order->shipping_last_name }}</td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-sm text-gray-800">{{ $order->shipping_address_line2 }}</td>
                                        </tr>
                                        @endif
                                        <tr>
            ...
            <div class="col-span-1 lg:col-span-1">
                <div class="card grow shadow-lg rounded-lg bg-white">
                    ...
                    <div class="card-body pt-4 pb-3">
                        <form action="{{ route('order.updateStatus', $order->id) }}" method="POST" class="space-y-4">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="w-full mt-4">
                                <label for="general_status" class="block text-sm font-medium text-gray-700 mb-1">General Status</label>
                                <select name="general_status" id="general_status" class="select">
                                    <option value="">-- Select Status --</option>
                                    <option value="processing" {{ $order->general_status === 'processing' ? 'selected' : '' }}>Processing</option>
                                    <option value="completed" {{ $order->general_status === 'completed' ? 'selected' : '' }}>Completed</option>
                                    <option value="cancelled" {{ $order->general_status === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    <option value="failed" {{ $order->general_status === 'failed' ? 'selected' : '' }}>Failed</option>
                                </select>
                            </div>
                        ...
                        <table class="table-auto w-full text-sm text-gray-700">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm text-gray-800">
                                        <span class="icon-saudi_riyal"></span> {{ number_format($order->coupon_discount, 2) }}
                                    </td>
                                </tr>
                                <tr>
                                    <td class="text-sm text-gray-600 min-w-36 pb-3 pe-6">
                                    ...
                                    <td class="text-sm text-gray-800">
                                        @if($totalShippingFee)
                                            <span class="icon-saudi_riyal"></span> {{ number_format($totalShippingFee, 2) }}
                                        @else
                                            Free delivery
                                        @endif
                                    </td>
                            ...
                            <tbody>
                                @if($order->pickupPoint)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-sm text-gray-800">
                                            <a href="tel:{{ $order->pickupPoint->phone_number }}" class="text-primary hover:underline">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-sm text-gray-800">
                                            ...
                                            @else
                                                <span class="badge badge-sm badge-warning badge-outline">Inactive</span>
                                            @endif
                                        </td>
                                    </tr>
                                @else
