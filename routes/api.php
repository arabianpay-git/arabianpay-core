...
// [PHASE-0 2026-04-06] All financial/integration test routes gated behind
// local-environment check + auth:sanctum middleware (F-002).
...
// ─────────────────────────────────────────────
// Authenticated chat routes (unchanged)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// [PHASE-6] Health check endpoint (no auth)
// ─────────────────────────────────────────────
Route::get('/health', \App\Http\Controllers\HealthCheckController::class)->name('health');

// ─────────────────────────────────────────────
// [PHASE-5] ClickPay webhook callback (signature-verified, no auth middleware)
// ─────────────────────────────────────────────
Route::post('/webhooks/clickpay', [\App\Http\Controllers\Webhooks\ClickPayWebhookController::class, 'handle'])
    ->name('webhooks.clickpay');

// ─────────────────────────────────────────────
// Integration test routes: LOCAL ENVIRONMENT ONLY + auth:sanctum
// These routes expose SingleView, Nafith, and SIMAH services.
// They are disabled in all non-local environments.
// ─────────────────────────────────────────────
...
if (app()->environment('local')) {
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        // ── SingleView (Open Banking) test routes ──
        Route::prefix('singleview')->group(function () {
            ...
            Route::get('/accounts/{bankCode}/{consentId}', function ($bankCode, $consentId) {
                ...
                $ibanCheck = filter_var(request()->query('ibanCheck', true), FILTER_VALIDATE_BOOLEAN);
            ...
            Route::get('/all-transactions/{bankCode}/{consentId}', function ($bankCode, $consentId) {
                $service = app(SingleViewService::class);
                $fromDate = request()->query('fromDate', '2016-01-01T00:00:00+02:00');
                $toDate = request()->query('toDate', now()->toIso8601String());
        ...
        // ── Nafith (SANAD / Promissory Notes) test routes ──
        Route::prefix('nafith')->group(function () {
            ...
            Route::get('/auth-token', function (Request $request) {
                try {
                    ...
                    return response()->json(['success' => true, 'token' => $token, 'length' => strlen($token)]);
                } catch (\Exception $e) {
            ...
            Route::get('/create-sanad', function (Request $request) {
                try {
                    $nafithService = app(NafithService::class);
                    $sanadData = [
                        'debtor' => ['national_id' => $request->input('debtor_nin', '1000473387')],
            ...
            Route::get('/create-multiple-sanads', function (Request $request) {
                try {
                    ...
                    $sanadItems = [
                        ['due_type' => 'date', 'due_date' => '2025-12-28', 'total_value' => 200.00, 'reference_id' => 'sanad_multi_1_' . time()],
                        ['due_type' => 'upon request', 'total_value' => 920.44, 'reference_id' => 'sanad_multi_2_' . time()],
                    ];
                    $totalValue = 200.00 + 920.44;
                    $result = $nafithService->createMultipleSanads($debtorData, $sanadItems, $request->input('reference_id', 'test_multi_ref_' . time()), $request->input('city_of_payment', '1'), $totalValue);
                    return response()->json(['success' => true, 'data' => $result]);
                } catch (\Exception $e) {
                    return response()->json(['success' => false, 'error' => $e->getMessage()], 500);
                }
            ...
            Route::get('/create-sanad-with-creditor', function (Request $request) {
                try {
                    $nafithService = app(NafithService::class);
                    $creditorData = ['national_id' => $request->input('creditor_nin', '7001046324')];
                    $debtorData = ['national_id' => $request->input('debtor_nin', '1000473387'), 'phone_number' => $request->input('debtor_phone_number', '0546258295')];
                    $sanadItems = [
                        ['due_type' => 'date', 'due_date' => '2025-12-28', 'total_value' => 200.00, 'reference_id' => 'sanad4_' . time()],
                        ['due_type' => 'upon request', 'total_value' => 920.44, 'reference_id' => 'sanad5_' . time()],
                    ];
                    $totalValue = 200.00 + 920.44;
                    $result = $nafithService->createSanadWithCreditor($creditorData, $debtorData, $sanadItems, $request->input('reference_id', 'test_creditor_ref_' . time()), $request->input('city_of_issuance', 1), $request->input('city_of_payment', 2), $totalValue, $request->input('country_of_issuance', 'SA'), $request->input('country_of_payment', 'SA'));
                    return response()->json(['success' => true, 'data' => $result]);
                } catch (\Exception $e) {
                    return response()->json(['success' => false, 'error' => $e->getMessage()], 500);
                }
            ...
            Route::get('/close-individual-sanad/{sanadGroupId}', function (Request $request, string $sanadGroupId) {
                ...
                if (empty($sanadId)) {
                    return response()->json(['success' => false, 'error' => 'sanad_id is required'], 400);
                }
                try {
                    $nafithService = app(NafithService::class);
            ...
            Route::get('/get-sanad-by-number', function (Request $request) {
                try {
                    $nafithService = app(NafithService::class);
                    $sanadNumber = $request->input('sanad_number', '10211025262474');
            ...
            Route::get('/download-sanad-group', function (Request $request) {
                try {
                    $nafithService = app(NafithService::class);
                    $sanadGroupId = $request->input('sanad_group', '83266b77-73cb-4c5d-a395-285a5cf7f293');
        ...
        // ── SIMAH (Credit Bureau) test routes ──
        Route::get('/test-silver-report', function () {
            ini_set('max_execution_time', 300);
            try {
                $simahService = app(SimahService::class);
                $data = [
                    'idNumber' => '7035087050', 'nationality' => 196, 'familyName' => 'ABC',
                    'firstName' => 'ABB', 'secondName' => 'BBC', 'thirdName' => 'CCD',
                    'expiryDate' => '30/10/2040', 'gender' => 1, 'dateOfBirth' => '30/11/1970',
        ...
        Route::get('/test-consumer-score', function () {
            ...
            try {
                ...
                $data = [
                    'language' => 'en',
                    'identityInfo' => ['idType' => 2, 'idNumber' => '2583103284', 'productId' => 23],
                    'applicationDetails' => ['amount' => 100, 'productType' => 23],
                    'demographicInfo' => [
                        'isHijriIDExpiryDate' => true, 'idExpiryDate' => '30/05/1453',
                        'nationality' => 168, 'maritalStatus' => 1, 'isHijriDateOfBirth' => true,
                        'dateOfBirth' => '09/06/1930', 'firstName' => 'Asad', 'gender' => 1,
                        'secondName' => 'Mahmood', 'thirdName' => 'third', 'familyName' => 'family',
                    ],
                    'accept' => true,
                ];
    ...
    // Non-local: catch any attempt to hit financial test API routes and log + deny
    Route::any('/singleview/{any?}', function () {
        Log::warning('[PHASE-0] Blocked non-local access to SingleView test API', ['ip' => request()->ip()]);
        abort(404);
    })->where('any', '.*');

    Route::any('/nafith/{any?}', function () {
        Log::warning('[PHASE-0] Blocked non-local access to Nafith test API', ['ip' => request()->ip()]);
        abort(404);
    })->where('any', '.*');

    Route::any('/test-silver-report', function () {
        Log::warning('[PHASE-0] Blocked non-local access to SIMAH test API', ['ip' => request()->ip()]);
        abort(404);
    });

    Route::any('/test-consumer-score', function () {
        Log::warning('[PHASE-0] Blocked non-local access to SIMAH test API', ['ip' => request()->ip()]);
        abort(404);
    });
}
