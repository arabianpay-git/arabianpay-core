        ...
        foreach ($days as $d) {
            ...
            foreach ($schedules as $schedule) {
                $this->sendUniqueReminder($schedule, "pre_{$d}", $targetDate, function ($schedule) use ($d, $targetDate) {
                    ...
                    $notificationBody = "Your scheduled payment #{$schedule->id} is due on {$schedule->due_date}.";
                    $notificationData = [
                        ...
                        'days_left' => $d,
                        // any other useful fields can be added here
                    ];

                    $this->dispatchNotification($schedule->user?->id ?? null, $notificationTitle, $notificationBody, $notificationData);
                });
            }
        }
    }
    ...
    protected function sendDailyDueReminders(\Carbon\Carbon $date)
    {
        ...
        foreach ($schedules as $schedule) {
            ...
            $this->sendUniqueReminder($schedule, 'due_daily', $date->toDateString(), function ($schedule) use ($attemptNumber, $date) {
                ...
                $notificationBody = "Your scheduled payment #{$schedule->id} is due (attempt {$attemptNumber}).";
                $notificationData = [
                    'mobile_screen' => 'schedule_payment',
                    'reference_id' => (string)$schedule->id,
                    'schedule_payment_id' => (string)$schedule->id,
